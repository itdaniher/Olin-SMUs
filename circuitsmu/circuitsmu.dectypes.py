...
class smu():
	...
	def __init__(self, num = 0):
		...
		self.dev.ctrl_transfer(0x00, 0x09, 1, 0, self.buffer)
		self.SET_FN = 0
		self.GET_FN = 1
	...
	def set_autorange(self, ch, arange):
		...
		if (ch==1) or (ch==2):
			if (arange<0) or (arange>1):
				...
				buffer = self.dev.ctrl_transfer(0x40, self.SET_AUTORANGE, arange, ch, self.buffer)
		else:
			print "Illegal channel number specified.\n"
	...
	def set_function(self, ch, fn):
		...
		if (ch==1) or (ch==2):
			if (fn<0) or (fn>1):
				...
				buffer = self.dev.ctrl_transfer(0x40, self.SET_FN, fn, ch, 0)
		else:
			print "Illegal channel number specified.\n"
	...
	def set_irange(self, ch, irange):
		...
		if (ch==1) or (ch==2):
			if (irange<0) or (irange>6):
				...
				buffer = self.dev.ctrl_transfer(0x40, self.SET_IRANGE, irange, ch, self.buffer)
		else:
			print "Illegal channel number specified.\n"
		...
		if (ch==1) or (ch==2):
			if (units<0) or (units>1):
				...
				if range!=self.OVER_RANGE:
					...
					buffer = self.dev.ctrl_transfer(0x40, self.SET_SRC, temp, (units<<8)|ch, self.buffer)
		else:
			print "Illegal channel number specified.\n"
	...
	def set_vrange(self, ch, vrange):
		...
		if (ch==1) or (ch==2):
			if (vrange<0) or (vrange>3):
				...
				buffer = self.dev.ctrl_transfer(0x40, self.SET_VRANGE, vrange, ch, self.buffer)
		else:
			print "Illegal channel number specified.\n"
