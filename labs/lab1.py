import smu, pylab, pickle

smu = smu.smu()

data = {}

for v in pylab.arange(1, 5, .05):
	smu.set_voltage(1, v)
...
for a in pylab.arange(.0001, .01, .00005):
    smu.set_voltage(2, 0)
    smu.set_current(1, a)
...
def fig3Alt():
	clean = pickle.load(open("exp3.p"))
	pylab.plot(clean.keys(), clean.values(), 'k.', label='raw experimental data')
	pylab.legend(loc = 'upper left')
	pylab.xlabel('$I_{out}$ (amps)')
	pylab.ylabel('$I_{in}$ (amps)')
	pylab.title("Experiment 3 - Raw Data")
...
def fig4():
	...
	pylab.xticks([-1, 0, 1, 2, 3, 4])
	pylab.yticks([1e-5, 1e-4, 1e-3])
